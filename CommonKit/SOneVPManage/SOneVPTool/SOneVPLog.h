...
syslog(LOG_CRIT, "%s(line %d) %s:", __func__, __LINE__, logFmt, category,##__VA_ARGS__); \
...
syslog(LOG_CRIT, "%s, %s(line %d) %s:"logFmt, [[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __func__, __LINE__ , category,##__VA_ARGS__); \
//syslog(LOG_CRIT, "%s:" logFmt, category,##__VA_ARGS__); \
