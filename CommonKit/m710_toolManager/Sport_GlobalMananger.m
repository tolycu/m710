...
@interface Sport_GlobalMananger ()<CNContactPickerDelegate,CNContactViewControllerDelegate,MFMailComposeViewControllerDelegate>
...
@implementation Sport_GlobalMananger

+ (instancetype)sharedInstance{
    static dispatch_once_t onceToken;
    static Sport_GlobalMananger *tool;
...
- (void)startShareString:(NSString *)string{
    //调用系统的分享
    NSArray *activityItemsArray = @[@"ScannerLab",string];
...
- (void)startShareImage:(UIImage *)image{
    //调用系统的分享
    NSArray *activityItemsArray = @[@"ScannerLab",image];
...
- (void)sendEmailAddress:(NSString *)address {
    ...
    [mailVC setSubject:@"[FaceArt] Feedback"];
...
- (void)saveMeCardNewContact:(NSString *)string{
    ...
    Sport_CardModel *model = [Sport_CardModel mj_objectWithKeyValues:mutDic];
    ...
    [self setSport_CardModel:model ForContact:contact existContect:NO];
...
//设置要保存的contact对象
- (void)setSport_CardModel:(Sport_CardModel *)model ForContact:(CNMutableContact *)contact existContect:(BOOL)exist{
    if (!exist) {
        NSArray *nameArr;
...
- (void)showSelectPlatform:(NSString *)string{
    ...
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"" message:@"Choose one to open this product." preferredStyle:[Sport_GlobalMananger checkoutDeviceIsiPad]?UIAlertControllerStyleAlert:UIAlertControllerStyleActionSheet];
    ...
    UIAlertAction *google = [UIAlertAction actionWithTitle:@"Google Shopping" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        XCWebViewController *vc = [XCWebViewController new];
