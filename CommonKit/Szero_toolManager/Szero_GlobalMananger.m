...
@interface Szero_GlobalMananger ()<CNContactPickerDelegate,CNContactViewControllerDelegate,MFMailComposeViewControllerDelegate>
...
@implementation Szero_GlobalMananger

+ (instancetype)sharedInstance{
    static dispatch_once_t onceToken;
    static Szero_GlobalMananger *tool;
...
+ (NSString *)qrTypeString:(DataStringQRType)type{
    if (type == DataStringQRType_text) {
        ...
        return @"barcode";
    }else if (type == DataStringQRType_FB){
        return @"facebook";
...
- (void)saveMeCardNewContact:(NSString *)string{
    ...
    Szero_CardModel *model = [Szero_CardModel mj_objectWithKeyValues:mutDic];
    ...
    [self setSzero_CardModel:model ForContact:contact existContect:NO];
...
//设置要保存的contact对象
- (void)setSzero_CardModel:(Szero_CardModel *)model ForContact:(CNMutableContact *)contact existContect:(BOOL)exist{
    if (!exist) {
        NSArray *nameArr;
...
- (void)showSelectPlatform:(NSString *)string{
    ...
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"" message:@"Choose one to open this product." preferredStyle:[Szero_GlobalMananger checkoutDeviceIsiPad]?UIAlertControllerStyleAlert:UIAlertControllerStyleActionSheet];
