...
- (void)post:(NSString *)api
    ...
    // 加密操作
    NSString *dataString = [AESencrytDecrypt aesPostWithDict:params];
    NSData *encrytData = [AESencrytDecrypt encryptAES:dataString key:XC_EncryptKey vector:XC_EncryptInitVector keySize:kCCKeySizeAES128];
...
//    [requestManager.requestSerializer setValue:@"test" forHTTPHeaderField:@"domain"];
    ...
    NSURLSessionDataTask * task = [requestManager dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        ...
        NSData *responseData = [AESencrytDecrypt decryptAES:unZipData key:XC_DecryptKey vector:XC_DecryptInitVector keySize:kCCKeySizeAES128];
